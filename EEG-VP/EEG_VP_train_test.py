...
def my_normalize(data_array):
    ...
    if isinstance(data_array, str):
        # 如果输入是字符串,先转换为数值数组
        data_array = np.fromstring(data_array, sep=',')
    else:
    ...
    # 确保数组大小正确
    expected_size = C * T
    if data_array.shape[1] != expected_size:
        raise ValueError(f"输入数据维度不正确。期望的特征数量为 {expected_size}，但得到了 {data_array.shape[1]}")
    
    # 归一化处理
    normalize = StandardScaler()
    normalized_data = normalize.fit_transform(data_array)
    
    # 重塑为所需的形状
    return normalized_data.reshape(-1, C, T)
...
def train(net, train_iter, val_iter, test_iter, num_epochs, lr, device, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 3 == 0:
            # 计算当前的训练损失
            current_train_loss = metric[0] / metric[2] if metric[2] != 0 else float('inf')
            # 计算当前的训练准确率
            current_train_acc = metric[1] / metric[2] if metric[2] != 0 else 0
            
            print(f'loss {current_train_loss:.3f}, train acc {current_train_acc:.3f}, val acc {val_acc:.3f}, test acc {test_acc:.3f}')
    ...
    for test_set_id in range(7):
        ...
        with torch.no_grad():
            for X, y in test_iter:
                ...
                all_test_pred = np.concatenate((all_test_pred, y_pred))
                
            all_test_label = np.concatenate((all_test_label, test_label))
        
        # 计算并记录准确率
        top_1_acc = np.mean(block_top_1)
        top_k_acc = np.mean(block_top_k)
