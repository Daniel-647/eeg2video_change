...
for subname in range(1,21):
    ...
    # 确保保存目录存在
    os.makedirs("../data/EEG2Video/DE_1per2s", exist_ok=True)
    os.makedirs("../data/EEG2Video/PSD_1per2s", exist_ok=True)
    
    # 使用相对路径保存文件
    np.save(f"../data/EEG2Video/DE_1per2s/sub{subname}.npy", DE_data)
    np.save(f"../data/EEG2Video/PSD_1per2s/sub{subname}.npy", PSD_data)